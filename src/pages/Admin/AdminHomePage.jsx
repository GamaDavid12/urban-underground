...
const AdminHomePage = () => {
    ...
    const fetchHotspots = async () => {
        ...
        setError(null);
        try {
            const response = await fetch(API_URL_HOTSPOTS);
            if (!response.ok) {
                throw new Error(`Fallo al obtener los hotspots: ${response.statusText}`);
            }
    ...
    const fetchProducts = async () => {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(`Fallo al obtener los productos: ${response.statusText}`);
            }
            ...
            if (Array.isArray(result.products)) {
                ...
                throw new Error("La respuesta de /api/products no contiene una matriz 'products' válida.");
            }
        } catch (err) {
            console.error("Error en la carga inicial de productos:", err);
            setError(prev => (prev ? prev + ` | Error de la API (Productos): ${err.message}` : `Error de la API (Productos): ${err.message}`));
        } finally {
    ...
    const saveHotspot = async (hotspotToSave) => {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(`Fallo ${method}: ${response.status} - ${errorData.message || response.statusText}`);
            }
            ...
            if (isNew) {
                ...
                setHotspots(prev => prev.map(h => String(h.id) === String(id) ? savedHotspot : h));
            }

            setEditingHotspot(null);
            console.log(`Hotspot ${isNew ? 'creado' : 'actualizado'} con éxito.`, savedHotspot);
        } catch (err) {
    ...
    const deleteHotspot = async (hotspotId) => {
        ...
        try {
            ...
            setHotspots(prev => prev.filter(h => String(h.id) !== String(hotspotId)));
            setEditingHotspot(null);
    ...
    return (
        <div className="p-4 md:p-8 bg-gray-900 text-white min-h-screen"> 
            ...
            <p className="text-gray-400 mb-4">
                Haz clic en cualquier punto para editarlo. Un Hotspot temporal se puede arrastrar en el panel de edición.
            </p>

            {isLoading && (
                <div className="bg-blue-600 p-3 rounded mb-4 text-center">
                    Cargando {isLoadingHotspots ? 'Hotspots' : ''} {isLoadingProducts ? 'Productos' : ''}...
                </div>
            )}
            
            {error && (
                <div className="bg-red-700 p-3 rounded mb-4 font-bold">⚠️ Error de la API: {error}</div>
            )}

            {!isLoadingProducts && allProducts.length === 0 && (
                <div className="bg-orange-500 p-3 rounded mb-4 font-bold">
                    🚨 Advertencia: No se encontraron productos en el backend. Asegúrate de que el seeder de productos se ha ejecutado y la ruta /api/products funciona correctamente.
                </div>
            )}
            ...
            > 
                ...
                <img 
                    ...
                    onError={(e) => { e.target.onerror = null; e.target.src = "https://placehold.co/1200x800/222/FFF?text=FALTA IMAGEN DE INICIO"; }}
                />
                
                {hotspots.map((hotspot) => {
                    const productData = allProducts.find(p => String(p.id) === String(hotspot.productId));
                    const isBeingEdited = editingHotspot && (String(editingHotspot.id) === String(hotspot.id));
                    ...
                    return (
                        <Hotspot 
                            ...
                            top={hotspot.top} 
                            left={hotspot.left} 
                            product={productData || { id: hotspot.productId, name: `Producto ID ${hotspot.productId} (No encontrado)` }} 
                            onClick={(e) => {
                                e.stopPropagation();
                ...
                {editingHotspot && editingHotspot.isNew && (
                    <Hotspot 
                        ...
                        product={allProducts.find(p => String(p.id) === String(editingHotspot.productId)) || { id: 0, name: 'SELECCIONAR PRODUCTO' }} 
                        onClick={() => {}}
                        onMove={handleHotspotMove}
                        isAdmin={true}
                        isEditing={true}
...
const Button = ({
  ...
  const baseClasses = "flex w-full justify-center font-bold items-center border rounded-md p-2 max-h-40 p3 gap-x-2 cursor-pointer transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed";
  
  const variants = {
    based: "border-[#FFCA1E] text-[#FFCA1E] bg-inherit hover:bg-[#FFCA1E] hover:text-gray-950",
    icon: "!w-auto border-none text-[#FFCA1E] text-2xl bg-inherit hover:text-white",
    outlined: "border-[#FFCA1E] text-[#FFCA1E] bg-inherit hover:bg-[#FFCA1E] hover:text-gray-950",
    contained: "bg-[#FFCA1E] border-[#FFCA1E] text-gray-950 hover:bg-yellow-500",
    ...
    gradient: "bg-gradient-to-r from-[#FFCA1E] to-black border-none text-white hover:bg-gradient-to-r hover:from-yellow-400 hover:to-gray-800",
  };

  const finalClasses = `${baseClasses} ${variants[variant] || variants.contained} ${className} ${
